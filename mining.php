...
{
    // Menghapus data pada tabel itemset1
    $sql1 = "TRUNCATE itemset1";
    $db_object->db_query($sql1);

    // Menghapus data pada tabel itemset2
    $sql2 = "TRUNCATE itemset2";
    $db_object->db_query($sql2);

    // Menghapus data pada tabel itemset3
    $sql3 = "TRUNCATE itemset3";
    $db_object->db_query($sql3);

    // Menghapus data pada tabel confidence
...
function reset_hitungan($db_object, $id_process)
{
    ...
    // Menghapus record pada tabel itemset1 berdasarkan id_process
    $db_object->delete_record("itemset1", $condition);

    //$condition = array("id_process"=>$id_process);
    // Menghapus record pada tabel itemset2 berdasarkan id_process
    $db_object->delete_record("itemset2", $condition);

    //$condition = array("id_process"=>$id_process);
    // Menghapus record pada tabel itemset3 berdasarkan id_process
    $db_object->delete_record("itemset3", $condition);

    //$condition = array("id_process"=>$id_process);
    // Menghapus record pada tabel confidence berdasarkan id_process
...
function is_exist_variasi_itemset($array_item1, $array_item2, $item1, $item2)
{
    ...
    // Mencari apakah variasi itemset sudah ada dalam array
    ...
    // Memeriksa apakah variasi itemset sudah ada
...
function mining_process($db_object, $min_support, $min_confidence, $start_date, $end_date, $id_process)
{
    ...
    // Query untuk mengambil data transaksi dari tabel 'transaksi' berdasarkan rentang tanggal
    $sql_trans = "SELECT * FROM transaksi 
            WHERE transaction_date BETWEEN '$start_date' AND '$end_date' ";
    $result_trans = $db_object->db_query($sql_trans);
    // Inisialisasi array untuk menyimpan data transaksi dan item_list (belum digunakan dalam kode yang diberikan)
    ...
    // Menghitung nilai support minimum relatif
    $min_support_relative = ($min_support / $jumlah_transaksi) * 100;
    // Variabel yang tampaknya tidak digunakan dalam kode yang diberikan
    $x = 0;
    // Iterasi untuk setiap baris hasil query
    while ($myrow = $db_object->db_fetch_array($result_trans)) {
        // Menyimpan data transaksi ke dalam array data Transaksi
        ...
        //all items
        // Menyimpan produk ke dalam array item_list
        ...
        while ($b <= count($itemset2_var1)) {
            if ($a != $b) {
                ...
                if (!empty($itemset1a) && !empty($itemset1b) && !empty($itemset2a) && !empty($itemset2b)) {
                    ...
                    $temp_array = get_variasi_itemset3(
                        $tigaVariasiItem,
                        $itemset1a,
                        $itemset1b,
                        $itemset2a,
...
function hitung_confidence(
    ...
    // Menghitung jumlah kemunculan kombinasi dua item dalam itemset 2
    $jml_itemset2 = jumlah_itemset2($dataTransaksi, $atribut1, $atribut2);
    // Menghitung nilai support kombinasi dua item sebagai persentase dari jumlah transaksi
...
function hitung_confidence1(
    ...
    // Menghitung nilai support atribut1 sebagai persentase dari jumlah transaksi
    $nilai_support_x = ($jml_itemset1 / $jumlah_transaksi) * 100;

    // Menyimpan atribut pertama dari kombinasi dua itemset
    $kombinasi1 = $atribut1;
    // Menyimpan kombinasi atribut kedua dari dua itemset yang akan dihitung confidence-nya
    $kombinasi2 = $atribut2 . " , " . $atribut3;
    // Menyimpan nilai support dari atribut pertama
    $supp_x = $nilai_support_x;
    // Menghitung confidence antara kombinasi dua itemset
    $conf = ($supp_xuy / $supp_x) * 100;
    // Menentukan apakah kombinasi dua itemset ini lolos seleksi min confidence
    $lolos = ($conf >= $min_confidence) ? 1 : 0;
...
// Fungsi untuk menghitung confidence antara dua item dalam itemset
function hitung_confidence2(
    $db_object,
    $supp_xuy,
    $min_support,
    $min_confidence,
    ...
    // Variabel untuk menyimpan kombinasi item
    $kombinasi1 = $atribut1;
    $kombinasi2 = $atribut2;
    $supp_x = $nilai_support_x; //$row1_['support'];

    // Menghitung nilai confidence antara dua item
    $conf = ($supp_xuy / $supp_x) * 100;
    //lolos seleksi min confidence itemset3
    // Menentukan apakah nilai confidence lolos seleksi min confidence itemset3
    $lolos = ($conf >= $min_confidence) ? 1 : 0;
    ...
    // Memasukkan data ke dalam tabel confidence
    $db_object->insert_record(
        "confidence",
        array(
            "kombinasi1" => $kombinasi1,
...
// Fungsi untuk menghitung jumlah kemunculan suatu item dalam itemset 1
...
// Fungsi untuk menghitung jumlah kemunculan suatu kombinasi dua item dalam itemset 2
